...
- `core`: 音高、音程、拍子、音価、テンポ、音律、検証、エラー分類、ノートイベントのインデックス、乱数ユーティリティ、楽器プロファイル。
- `theory`: スケール、コード仕様、記号、綴り、機能規則、対位法、通奏低音、ボイシング。
- `analyze`: コード・調の検出、和声解析、タイムライン、縮約、旋律解析、形式、アレンジのレポート。
- `generate`: 生成コンテキストと複雑さのダイヤル、進行、ベース、対旋律、ドラム、グルーヴ、和音付け、モチーフ、装飾、リズム、リハーモナイズ、vocabulary。
- `model`: 不変のクラス API。他の層が扱う対象ごとに1クラスあり、いずれもその関数群の薄い外皮です。解析や生成の経路は、クラス API だけで最初から最後まで書けます。一部は意図して関数のままです。ホストが自分の境界で使う検証関数と型ガード、2音の対を見る対位法の述語、辞書・カタログの読み出し（`BASS_LICKS`、`DRUM_PATTERNS`、`progressions`）、シードの原始関数、そしてパートではなく工程である生成ステージ（`imitate`、`placeDrumPattern`、`placeLicks`）がそれにあたります。加えて、受け手はあるがメソッドがまだ無いものが少しあります（`barPositionToBeat`、`barPositionToPulse`、`chordFromSpec`、`secondaryDominant`、`shiftByScaleDegrees`）。いずれもクラスと並べてインポートします。
...
| 1つのアプリケーションを通した流れ | [ユースケース](use-cases/index.md) |
| 用語と、それを実装する API | [用語集](glossary.md) |
| 適用範囲、制限、設計上の判断 | [FAQ と制限](faq.md) |
...
英語ガイドの `ts` コードブロックはすべて抽出されてテストスイートで実行され、末尾の `// 値` コメントがリテラルを名指していれば（そのあとに理由が続いていてもかまいません）期待値として検証されます。日本語ページは同じブロックをそのまま持ち、同じスイートがその一致を検証します。ここに載っている例は、いずれも実行されたものです。
