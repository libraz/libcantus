...
- `model`: 不変のクラス API。他の層が扱う対象ごとに1クラスあり、いずれもその関数群の薄い外皮です。解析や生成の経路は、クラス API だけで最初から最後まで書けます。関数のままの補助もいくつかあり（`barPositionToBeat`、`barPositionToPulse`、`chordFromSpec`、`secondaryDominant`、`shiftByScaleDegrees`）、これらはクラスと並べてインポートします。
