...
離れる側のコードが手元にあるなら、7thを含むかどうかにかかわらず `previousChord` を渡します。離れる和音から読む規則は3つ — 7thの解決、導音の解決、そして2つの和音のあいだの交差関係 — あり、いずれもこれが無ければ採点されません。ボイシング自身は、どの和音の上に書かれたものかを語らないからです。渡せば3つとも `voiceProgression` と同じ基準で採点され、生成側と `checkPartWriting` が同じ対について同じことを言うようになります。
