...
- **生成されたパート**は同じ意味での派生データではありません。再現にはシード、解決後の `algorithmVersion`、ジェネレータの全オプション、そして出力したパッケージのバージョンが必要です。これらを保存し、可能なら出力されたノートも保存します。[決定性とシード](determinism-and-seeding.md)を参照してください。
