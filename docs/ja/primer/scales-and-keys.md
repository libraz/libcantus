...
`2741` は長音階の並びを表すマスク、`0b101010110101` です。調を受け取る入口はこの3つの形と `'C major'` のような文字列をすべて受け付け、`resolveKey` に通します。素の `KeyScale` が渡された場合は主音の綴りが選ばれます。臨時記号がいちばん少ないものが選ばれるので、`majorKey(6)` は嬰ヘ長調ではなく変ト長調になります。曲がもう一方で書かれている場合は、綴られた調を渡してください。
