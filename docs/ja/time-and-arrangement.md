...
拍子が変わる曲は `MeterMap`（`{ startBeat, ts }` の変化の配列）で表します。時間に依存する解析が受け取るのはこれです。`TimeSignature` を1つ渡すと、要素1つのマップの略記になります。拍0が強拍で、アウフタクトは負の拍から始められます。拍子が変わる曲は拍子区間ごとに分けて呼ばず、マップを付けて曲全体を1回で渡してください。区間ごとに呼ぶと、拍子の変わり目をまたぐ声部の動きや調域が見えなくなります。`Score` は自身のマップを持ち、位置はそれを通して読みます。
