...
この流れの出発点は、実音のノートイベント（ライブラリの他の部分が出力するもの）と楽器プロファイルです。ここで使う語彙、とくに移調楽器と、それに伴って移る調号については[入門](../primer/index.md)を参照してください。
...
`playability` は3つの層の障害を報告し、存在に関わるのは第1層だけです。この層には2種類の項目があります。`noteOutOfRange` は、その音が楽器にそもそも存在しないこと、`articulationUnavailable` は、書かれた奏法をその楽器が持たないことを意味します。音域外の音に対して、`Instrument.foldIntoRange` はその楽器が実際に鳴らせる最も近いオクターブを返します。最低弦より低く書かれたベースラインに対して、奏者が実際に行う対応と同じです。`Score.map` はパート全体をその折り返しに通し、拍子とテンポはそのまま保ちます。
