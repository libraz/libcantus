...
- **範囲** — `startBeat` と `endBeat`。UI は `Score.barAt` で小節番号に変換します。`barAt` は 0 起点なので、表示する前に 1 を足すか、印刷された楽譜と同じ 1 起点の `formatBarPosition` に拍を渡してください。2 つの起点については[時間とアレンジ](../time-and-arrangement.md)を参照してください。
