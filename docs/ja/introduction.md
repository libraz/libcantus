...
音楽の知識は前提としません。[入門](primer/index.md)では、このライブラリが扱う音楽の考え方を1つずつ説明し、それを実装している関数の名前を示します。このページに知らない用語があれば、そこから読んでください。

パッケージに実行時依存はなく、すべての入り口が同期的です。関数 API が返すのは JSON 互換のプレーンデータで、クラスインスタンスは現れません。クラス API は同じデータを包み、`.data` で公開します。

## ライブラリ全体の流れ

![解析に入るノートイベントとコード記号、そしてその読みに対して新しいパートを書く生成](../images/pipeline-ja.svg)

素材はノートイベントかコード記号として入ります。解析はそこから和音・調・終止形・フレーズといった構造を読み取り、生成はその読みに対して新しいパートを書きます。どの段階もプレーンデータを受け取ってプレーンデータを返すため、手元のデータに合う段階から始め、その結果だけを使って終えることもできます。
...
![下のレイヤーの上に積み重なる5つのレイヤー](../images/architecture-ja.svg)

レイヤーは積み重なっています。`theory` は `core` の上に、`analyze` は `theory` の上に、`generate` はその3つの上に、`model` はその4つすべての上に構築されています。上位のレイヤーを呼び出すことはないため、下のサブパスからインポートするほど取り込む範囲は小さくなります。サブパスからのインポートはパッケージングの選択であり、別の API ではありません。
...
**生成は再現可能です。** シード、アルゴリズムバージョン、パラメータが同じであれば、同じビルドから同じ音が得られます。プロジェクトにはその3つを記録します。乱数は呼び出し順ではなく位置で指定されるため、パラメータを1つ変えても、それ以降が引き直しになりません。

## エンジンの前提

ピッチ層より上の解析は12のピッチクラスで動作します。ピッチクラスとは、オクターブを捨てた音の識別子で、C が 0、B が 11 です。周波数、セント、オクターブの等分平均律、純正律の比は `core` で扱えますが、和声解析は微分音のピッチクラスをモデル化しません。

機能和声、つまり各和音を調のなかでトニック・サブドミナント・ドミナントのどの機能として読むかという見方は、それが成り立つ調性素材を対象とします。ローマ数字や終止の読みを適用する前に、`supportsFunctionalHarmony` で対象のスケールを確認できます。全音音階など機能和声が成り立たないスケールに対する正しい答えは、その解析が適用できないということです。

綴りが意味を持つ場面では、エンジンは綴りの情報、つまりその音をどの音名と変化記号で書くかを保ちます。減5度と増4度はピッチクラス上の距離が同じですが、書かれた音程としては別物です。そのため和声課題、対位法、旋律の綴りは、すべてを数値に還元せず、綴られた音または調の文脈を受け取ります。

## 次に読むページ

- [入門](primer/index.md) — このライブラリが扱う音楽の考え方と、それを実装する API の対応: [音高と音程](primer/pitch-and-intervals.md)、[音階と調](primer/scales-and-keys.md)、[和音](primer/chords.md)、[和声](primer/harmony.md)、[声部](primer/voices.md)、[リズムと拍子](primer/rhythm-and-meter.md)。
