...
このページが前提とする音楽用語（音階、度数、調号、主音）は[入門のスケールと調のページ](primer/scales-and-keys.md)で説明しています。

調はルートのピッチクラスとモードマスクの組であり、それがどう書かれるかは含みません。関係を扱う関数群がその部分を補います。これらは5度圏（調を調号の順に並べたもので、隣どうしは5度離れています）の上を移動し、主音をそこから読み戻すため、綴りが正しく保たれます。
...
音階形そのものは調の `variant` です。値は `'major'`、`'natural'`、`'harmonic'`、`'melodic'`、そして4つのいずれにも当てはまらない調のための `'modal'` です。その音階形を持つスケールを名前で指定すると設定され、解決済みの調から読み戻せます。`variantOfMask` は素のモードマスクから同じ読みを行います。ピッチクラスだけで届いた調に対して `resolveKey` が行うのもこの読みです。
...
スケールと一緒に渡された `variant` は、信用されるのではなく検査されます。マスクが持たない音階形に対して `resolveKey` と `Key.of` は `InvalidInputError` を投げます。長調のマスクの上で `'harmonic'` を名乗る調は、和声的短音階として印字されながら素のハ長調と等しく比較されてしまうためです。`assertKeyVariant` はこの検査だけを取り出したもので、保存しておいた調を組み立てる前に検証する用途に使えます。

`isSignatureKey` は、その調が自身の調号で書かれるかどうかを返します。自身の調号を持つのは7つの全音階旋法と、和声的短音階・旋律的短音階です。後者の上げられた度数は、短調の調号に対する臨時記号として印字されます。それ以外のスケールは同主長調・短調の調号を近似として借りるだけです。借りているスケールの綴りはスケール自身から決めることになります。その調号からは、書かれ方について何も導けないためです。
...
![1つの調とそのもっとも近い調の関係](../images/key-relations-ja.svg)

伸びている線はいずれも1手の移動です。平行調は調号を保って主音を動かし、同主調は主音を保って調号を変え、属調と下属調は5度圏を♯側・♭側へ1歩動きます。距離はこの歩数で数え、近い調どうしほど共有する素材が多くなります。両方の調が持つ和音が転調の起点になり得るのはそのためです。`relatedKeysOf` はこの4つに、属調と下属調それぞれの平行調を加えた6つを、決まった順で返します。
...
`KeyRegion` は区間、そこで効いている調、そして [0, 1] の `confidence` を持ちます。信頼度は、その区間のピッチクラス分布と調プロファイルの相関で、相関が負の場合はその値ではなく 0 として報告されます。

2つ目の区間からは `modulation` も持ちます。これは1つ前の区間の調に対する関係の名前です。何にも続かない最初の区間では欠けており、2つの調が名前の付いたどの関係にも当てはまらない場合も欠けています。値が `'same'` になることはありません。区間は1つの調が続く最大の範囲であり、連続する区間が同じ調を共有することはないからです。`pivot` は転調の起点になったコードを示します。これを埋めるのは、コードを入力に取る入口だけです。軸和音は両方の調で読めるコードであり、どのコードが鳴ったかを知らずには特定できません。転調は根拠に基づく提案であって事実ではないため、信頼度を提示し、ユーザーが読みを上書きできるようにします。
...
2つの経路で扱いが異なるオプションが1つあります。`profile` はピッチクラスプロファイル（主音から数えた12要素のベクトルで、各半音階度数がどれだけ強く鳴ると期待されるかを表します）を指定するもので、ノート経路では各スロットの採点そのものに使われます。`detectModulations` は各コードが調の中で担う役割で採点するため、どの調を報告するかにプロファイルは関与しません。それを決めるのはコードです。ただし各区間の `confidence`（前述の相関）の算出には、ノート経路とまったく同じように使われます。

組み込みのプロファイルは3つあり、`'krumhansl'`、`'temperley'`、`'flat'` の名前で指定します。`'krumhansl'` は既定で、Krumhansl–Kessler のプローブトーン評定を持ちます。聴取者の判断であるため、どの半音階度数にもある程度の重みが残ります。`'temperley'` は Kostka–Payne コーパスの出現割合を持ち、半音階度数は 0 に近くなります。全音階的な音楽には決定力があり、半音階的な音楽には鈍くなります。`'flat'` は音階内の度数をすべて等しく、半音階度数を 0 とするため、順位付けは音階への所属だけに帰着します。自前の `{ major, minor }` の組も同じオプションで渡せます。各ベクトルはそれぞれの主音から数え、すべての要素が有限の数である必要があります。
