...
Two labels name borrowed chords rather than degrees of the key, and those are stated as the fixed offsets they mean: `modal` is the ♭VII a whole tone below the tonic whatever the key writes on its seventh degree, and the Phrygian bass falls a semitone onto the dominant from the lowered submediant.
