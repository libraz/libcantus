...
A conflict raised against another part — a vertical clash, a parallel or hidden perfect, a crossing — carries `partners`: the `trackIndex` and `noteId` of each note it was raised against, with the pairwise `reasons` that note contributes, so a panel can draw the line between the two notes rather than mark one of them. `evaluateSafety` gives the same breakdown by index into `otherVoices` when asked with `{ partners: true }`.
